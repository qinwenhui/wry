...
$(function(){
    ...
    function getAllItemList(){
        $.ajax({
            url: wryConfig.SERVER_URL+'/api/hOrder/queryAllHOrderVo.action',
            type: 'get',
            dataType: 'json',
    ...
    function fillList(data){
    	...
    	for(var i=0;i<data.length;i++){
    		...
    		currentNumber++;
    		html += '<tr><td><div class="checkbox"><label><input type="checkbox" id="checkbox'+i+'" value="'+data[i].id+'" aria-label="..."></label></div></td><td>'+data[i].id+'</td><td>'+data[i].user.username+'</td><td>'+data[i].waiter.nickname+'</td><td>￥'+data[i].price+'</td><td>'+state+'</td><td>'+data[i].satisfaction+'</td><td>'+format(data[i].transactionTime)+'</td><td><button type="button" class="btn btn-default" id="InfoBtn'+i+'" value="'+data[i].id+'">详情</button>&nbsp;&nbsp;<button type="button" class="btn btn-default" id="deleteBtn'+i+'" value="'+data[i].id+'">删除</button></td></tr>'
    	}
    	trDiv.html(html);
    ...
    function setOnClick(){
    	...
    	$('#seachBtn').click(function(){
    		...
    		//进行模糊查询
    		$.ajax({
                url: wryConfig.SERVER_URL+'/api/hOrder/queryHOrderVoByByPrice.action?minPrice='+minPrice+'&maxPrice='+maxPrice,
                type: 'get',
                dataType: 'json',
                success: function(data){
                	if(data.code == 0){
    ...
    function deleteOne(id){
    	$.ajax({
            url: wryConfig.SERVER_URL+'/api/hOrder/deleteHOrder.action?id='+id,
            type: 'get',
            dataType: 'json',
            success: function(data){
            	if(data.code == 0){
    ...
    function getOrderByState(query_state){
    	$.ajax({
            url: wryConfig.SERVER_URL+'/api/hOrder/queryHOrderVoByState.action?state='+query_state,
            type: 'get',
            dataType: 'json',
            success: function(data){
            	if(data.code == 0){
    ...
    function infoBtnOnClick(){
    	for(var i=0;i<currentNumber;i++){
    		$('#InfoBtn'+i).click(function(){
    			...
    			var id = $(this).val();
    			$.ajax({
    	            url: wryConfig.SERVER_URL+'/api/hOrder/queryHOrderVoById.action?id='+id,
    	            type: 'get',
    	            dataType: 'json',
    	            success: function(data){
    	            	if(data.code == 0){
