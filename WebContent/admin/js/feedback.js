...
$(function(){
    ...
    function getAllItemList(){
        $.ajax({
            url: wryConfig.SERVER_URL+'/api/feedback/getAllFeedbackVo.action',
            type: 'get',
            dataType: 'json',
    ...
    function setOnClick(){
    	...
    	$('#seachBtn').click(function(){
    		...
    		//进行模糊查询
    		$.ajax({
                url: wryConfig.SERVER_URL+'/api/feedback/getFeedbackVoLikeKeyword.action?keyword='+keyword,
                type: 'get',
                dataType: 'json',
                success: function(data){
                	if(data.code == 0){
    	...
    	$(document).click(function(){
    		$("#feedbackInfo-div").hide(200);
    	})
    	//点击提示框不隐藏（阻止冒泡事件）
    	$("#feedbackInfo-div").click(function(event){
    ...
    function deleteOne(id){
    	$.ajax({
            url: wryConfig.SERVER_URL+'/api/feedback/deleteFeedback.action?id='+id,
            type: 'get',
            dataType: 'json',
            success: function(data){
            	if(data.code == 0){
    ...
    function infoBtnOnClick(){
    	for(var i=0;i<currentNumber;i++){
    		$('#InfoBtn'+i).click(function(){
    			...
    			var id = $(this).val();
    			$.ajax({
    	            url: wryConfig.SERVER_URL+'/api/feedback/getFeedbackVoById.action?id='+id,
    	            type: 'get',
    	            dataType: 'json',
    	            success: function(data){
    	            	if(data.code == 0){
    	            		//获取详情成功，填充到详情的提示框
    	            		$('#feedbackContent').text(data.data.content);
    	            		$('#feedbackUsername').text(data.data.user.username);
    	            		//显示详情框
    	            		$('#feedbackInfo-div').show(200);
    	            	}else{
    	            		alert(data.msg)
    	            	}
    	                console.log(data);
