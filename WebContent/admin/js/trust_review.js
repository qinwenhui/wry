...
$(function(){
    ...
    function getAllItemList(){
        $.ajax({
            url: wryConfig.SERVER_URL+'/api/trustOrderReview/queryAllReview.action',
            type: 'get',
            dataType: 'json',
    ...
    function setOnClick(){
    	...
    	$('#seachBtn').click(function(){
    		...
    		//进行模糊查询
    		$.ajax({
                url: wryConfig.SERVER_URL+'/api/trustOrderReview/queryReviewByKeyword.action?keyword='+keyword,
                type: 'get',
                dataType: 'json',
                success: function(data){
                	if(data.code == 0){
    ...
    function deleteOne(id){
    	$.ajax({
            url: wryConfig.SERVER_URL+'/api/trustOrderReview/deleteReview.action?id='+id,
            type: 'get',
            dataType: 'json',
            success: function(data){
            	if(data.code == 0){
    ...
    function infoBtnOnClick(){
    	for(var i=0;i<currentNumber;i++){
    		$('#InfoBtn'+i).click(function(){
    			...
    			var id = $(this).val();
    			$.ajax({
    	            url: wryConfig.SERVER_URL+'/api/trustOrderReview/queryReviewById.action?id='+id,
    	            type: 'get',
    	            dataType: 'json',
    	            success: function(data){
    	            	if(data.code == 0){
