...
@RequestMapping("/api/trustOrderReview")
@Controller
public class TOReviewController {

	@Autowired
	private TrustOrderReviewService service;
	...
	public String queryAllReview() throws Exception{
		...
		List<TOReviewVo> reviewVos = service.queryAllReview();
		if(reviewVos.size() == 0){
			//没有查到相关数据
	...
	public String queryReviewByOrderId(int order_id) throws Exception{
		BaseJson baseJson = null;
		TOReviewVo reviewVo = service.queryReviewByOrderId(order_id);
		if(reviewVo == null){
			//没有查到相关数据
			baseJson = new BaseJson(1,"暂无数据",null);
		}else{
	...
	public String queryReviewById(int id) throws Exception{
		BaseJson baseJson = null;
		TOReviewVo reviewVo = service.queryReviewById(id);
		if(reviewVo == null){
			//没有查到相关数据
			baseJson = new BaseJson(1,"暂无数据",null);
		}else{
	...
	public String queryReviewByUserId(int user_id) throws Exception{
		BaseJson baseJson = null;
		List<TOReviewVo> reviewVos = service.queryByUserId(user_id);
		if(reviewVos.size() == 0){
			//没有查到相关数据
			baseJson = new BaseJson(1,"暂无数据",null);
		}else{
	...
	public String queryReviewByKeyword(String keyword) throws Exception{
		BaseJson baseJson = null;
		List<TOReviewVo> reviewVos = service.queryByKeyword(keyword);
		if(reviewVos.size() == 0){
			//没有查到相关数据
			baseJson = new BaseJson(1,"暂无数据",null);
		}else{
	...
	@ResponseBody
	public String addReview(TOReview review) throws Exception{
		BaseJson baseJson = null;
